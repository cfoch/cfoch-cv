    ...
    std::vector<cv::Rect> & track_windows)
{
  ...
  for (t = 0; t < n_track_windows; t++)
    _find_minimum_pair (distance_matrix, n_track_windows, n_landmarks, ignore,
        t);

  cout << "Pairs bef: " << endl;
  __print_matrix (ignore, n_track_windows, 2);
  cout << " -o-o-o-o- " << endl;

  // Sort "Ignore/Closest pairs values" by the index of the track_window".
  // qsort (ignore, n_track_windows, 2 * sizeof (int), _ignore_comparator);
  ...
  std::vector<std::vector<cv::Point>> tmp_landmarks (n_track_windows);
  for (i = 0; i < n_track_windows; i++)
    tmp_landmarks[ignore[i][0]] = new_landmarks[ignore[i][1]];

  for (i = 0; i < n_track_windows; i++)
    new_landmarks[i] = tmp_landmarks[i];
  /*
  for (i = 0; i < n_track_windows; i++) {
    std::vector<cv::Point> tmp_landmark;
    tmp_landmark = new_landmarks[ignore[i][0]];
    new_landmarks[ignore[i][0]] = new_landmarks[ignore[i][1]];
    new_landmarks[ignore[i][1]] = tmp_landmark;
  }
  */
